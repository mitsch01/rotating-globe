...
export function Globe({ globeConfig, data }) {
  ...
  const globeReady = useRef(false) // To track when the globe is fully initialized
  ...
  useEffect(() => {
    if (globeRef.current && globeData) {
      // Ensure that the globe is initialized before applying scaling
      const geometry = globeRef.current.geometry

      if (geometry && geometry.attributes.position) {
        // Apply scaling to the globe
        globeRef.current.scale.set(5, 5, 5) // Adjust these values to scale the globe
        globeReady.current = true // Mark the globe as ready
      }
    }
  }, [globeData])
...
export function World(props) {
  ...
  return (
    <Canvas style={{ width: "800px", height: "600px" }} scene={scene} camera={new PerspectiveCamera(45, aspect, 0.1, 2000)}>
      <WebGLRendererConfig />
